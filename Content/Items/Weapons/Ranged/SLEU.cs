...
namespace EventHorizons.Content.Items.Weapons.Ranged
{
    public class SLEU : ModItem
    {
        public override void SetDefaults()
        {
            ...
            // Gun Properties
            Item.shoot = ProjectileType<Waves>(); // For some reason, all the guns in the vanilla source have this.
        ...
        public override bool? UseItem(Player player)
        {
            ...
            PunchCameraModifier modifier = new(player.Center, (Main.rand.NextFloat() * ((float)Math.PI * 2f)).ToRotationVector2(), 10f, 3f, 10, 500f, FullName);
        ...
        {
            player.velocity = player.DirectionTo(Main.MouseWorld) * -4f;
            const int NumProjectiles = 8; // The humber of projectiles that this gun will shoot.

            for (int i = 0; i < NumProjectiles; i++)
            {
                // Rotate the velocity randomly by 30 degrees at max.
                Vector2 newVelocity = velocity.RotatedByRandom(ToRadians(15));
